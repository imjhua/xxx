...
function Calendar({
  ...
  const MONTHS = React.useMemo(() => {
    let locale: Pick<Locale, 'options' | 'localize' | 'formatLong'> = ko
  ...
  const disableLeftNavigation = () => {
    ...
    if (props.month && typeof props.month.getMonth === 'function') {
      return (
  ...
  return (
    <DayPicker
      ...
      className={cn('p-3 w-fit text-body2', className)}
      classNames={{
        months: 'flex flex-col sm:flex-row space-y-4 sm:space-y-0 justify-center',
        month: 'flex flex-col space-y-4 w-full',
        month_caption: 'flex justify-center pt-1 relative items-center h-10 mb-2',
        caption_label: 'text-sm font-medium',
        nav: 'space-x-1 flex items-center',
        button_previous: cn(
          buttonVariants({ variant: 'outline' }),
          'h-8 w-8 min-w-8 bg-transparent p-0 opacity-50 hover:opacity-100 absolute left-6 top-4 z-10',
          disableLeftNavigation() && 'pointer-events-none opacity-30'
        ),
        button_next: cn(
          buttonVariants({ variant: 'outline' }),
          'h-8 w-8 min-w-8 bg-transparent p-0 opacity-50 hover:opacity-100 absolute right-6 top-4 z-10',
          disableRightNavigation() && 'pointer-events-none opacity-30'
        ),
        month_grid: 'w-full border-collapse space-y-0',
        weekdays: cn('flex w-full', props.showWeekNumber && 'justify-end'),
        weekday: 'text-muted-foreground text-center min-w-9 w-9 h-8 font-normal flex items-center justify-center',
        week: 'flex w-full',
        day: 'min-w-9 w-9 h-9 text-center text-body2 p-0 relative hover:bg-tweb-primary6 hover:text-primary focus-within:relative focus-within:z-20 flex items-center justify-center aria-selected:bg-tweb-primary1 aria-selected:text-tweb-white',
        day_button: cn(
          'min-w-9 w-9 h-9 p-0 font-normal aria-selected:opacity-100 flex items-center justify-center '
        ),
        range_end: 'day-range-end',
        selected: cn(
          'bg-tweb-primary1 text-tweb-white rounded-md focus:bg-primary focus:text-primary-foreground'
        ),
        today: 'bg-tweb-primary6 text-primary',
        outside:
          'day-outside text-muted-foreground opacity-50 aria-selected:bg-accent/50 aria-selected:text-muted-foreground aria-selected:opacity-30',
        disabled: 'text-muted-foreground opacity-50',
        range_middle: 'aria-selected:bg-tweb-primary6 aria-selected:text-primary',
        hidden: 'invisible',
        ...classNames
      }}
      components={{
        ...
        MonthCaption: ({ calendarMonth }) => {
          return (
            <div className="inline-flex gap-2 px-10 justify-center w-full">
              <Select
              ...
              >
                <SelectTrigger className="focus:bg-accent focus:text-accent-foreground w-fit gap-1 border-none p-1 h-8">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {YEARS.map((year) => (
...
const TimePicker = React.forwardRef<TimePickerRef, TimePickerProps>(
  ({ date, onChange, hourCycle = 24, granularity = 'second' }, ref) => {
    ...
    React.useEffect(() => {
      // 타임존 이슈 방지: 날짜 변경 시 AM/PM 상태 동기화
      if (date && hourCycle === 12) {
...
const DateTimePicker = React.forwardRef<Partial<DateTimePickerRef>, DateTimePickerProps>(
  (
    ...
    const handleMonthChange = (newDay: Date | undefined) => {
      ...
      // month가 유효한 Date 객체가 아닐 경우 현재 날짜로 폴백하여 getHours 에러 방지
      const currentMonth = month instanceof Date ? month : new Date()
      if (!defaultPopupValue) {
        newDay.setHours(currentMonth.getHours(), currentMonth.getMinutes(), currentMonth.getSeconds())
    ...
    /* FIXME: Popover 스타일은 따로 분리하는게 좋을 듯. (DateRangePicker 참조) */
    return (
      <Popover>
